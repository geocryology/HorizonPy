...
def SVF_discretized(azi, hor, plane_az, plane_dip, delta_phi, interpolation='linear'):
    ...
    azi = np.array(azi) if type(azi) != numpy.ndarray and hasattr(azi, '__len__') else azi
    hor = np.array(hor) if type(hor) != numpy.ndarray and hasattr(hor, '__len__') else hor
    azi = azi[argsort(azi)]
    hor = hor[argsort(azi)] # sorting to order by azimuth
    ...
    cos2theta = np.power(np.cos(theta_h * pi / 180), 2)
...
def angl_rotate(dip, rot):
    ...
    if (not hasattr(dip, '__len__') == hasattr(rot, '__len__')):
        if hasattr(rot, '__len__'):
            ...
            rot = np.array(rot) if type(rot) != numpy.ndarray else rot
        else:
            rot = np.array([rot]*len(dip))
            dip = np.array(dip) if type(dip) != numpy.ndarray else dip
    else:
    ...
    ## Do calculation        
    rotated_dip = (np.arcsin(np.sin(dip * pi / 180) * np.cos(rot * pi / 180))) 
    rotated_dip *= (180 / pi)
